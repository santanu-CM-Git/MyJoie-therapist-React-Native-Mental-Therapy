  ...
  return (
    <ScrollView style={styles.wrapper}>
      {getFaq.length > 0 ?
        ...
        /> :
        <View>
          <Text style={{ color: '#4D4B4B', fontFamily: 'Poppins-Medium', fontSize: responsiveFontSize(2), textAlign: 'center', }}>No accepted order yet</Text>
        </View>
      }
    </ScrollView>
  );
  ...
  const fetchNewOrders = () => {
    AsyncStorage.getItem('userToken', (err, usertoken) => {
      axios.get(`${API_URL}/api/driver/get-all-order-item`, {
        ...
        .then(res => {
          let userInfo = res.data.response.records;
          console.log(JSON.stringify(userInfo), 'fetch completed order')
          setFaq(userInfo)
          setIsLoading(false);
        })
        .catch(e => {
  ...
  return (
    <ScrollView style={styles.wrapper}>
      {getFaq.length > 0 ?
        ...
        /> :
        <View>
          <Text style={{ color: '#4D4B4B', fontFamily: 'Poppins-Medium', fontSize: responsiveFontSize(2), textAlign: 'center', }}>No completed order yet</Text>
        </View>
      }
    </ScrollView>
  );
...
const Declined = () => {
  ...
  const fetchNewOrders = () => {
    AsyncStorage.getItem('userToken', (err, usertoken) => {
      axios.get(`${API_URL}/api/driver/get-all-order-item`, {
        ...
        .then(res => {
          let userInfo = res.data.response.records;
          console.log(JSON.stringify(userInfo), 'fetch declined order')
          setFaq(userInfo)
          setIsLoading(false);
        })
        .catch(e => {
  ...
  return (
    <ScrollView style={styles.wrapper}>
      ...
      /> :
      <View>
        <Text style={{ color: '#4D4B4B', fontFamily: 'Poppins-Medium', fontSize: responsiveFontSize(2), textAlign: 'center', }}>No decline order yet</Text>
      </View>
    }
    </ScrollView>
  );
